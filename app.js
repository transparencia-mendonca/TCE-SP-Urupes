...
const DBN="tcesp-urupes-v6",STORE="dados",KEY="base";
...
function tceExpenseUrl(year){return `https://transparencia.tce.sp.gov.br/municipio/urupes/${year}/despesas`}
...
(async()=>{
  ...
  try{
    ...
    if(!hasUsableData(db)){
      let old=null;
      try{
        let raw=localStorage.getItem("tcesp-urupes-data")||localStorage.getItem("tcesp-data");
    ...
    try{
      await save();
      localStorage.removeItem("tcesp-urupes-data");
    }catch(e){
      initError=e;
